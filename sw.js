 ...
 */
const CACHE_VERSION = "salishmap-shell-v24";   // 2026-08-13a: THE 0.56 km STEEPNESS BAKE GOES LIVE. The wave-steepness rings were computed on the phone from the 2.2 km current overview; they are now read from a band pre-computed on the 0.005 deg (~560 m) grid and served from R2 (sscofs/steep/), with the 2.2 km client path kept as the OFFLINE fallback - so the layer has two ring densities depending on network state, by design. What it fixes is WHERE the map looks: at 2.2 km several tidal passes (Weynton, Gillard) have LAND at the centre of their square. Measured at Cattle Pass over 43 h it does NOT change WHEN the layer fires - 16 rough hours, 27 clean, 0 disagreements, peak band matched in all 16 - it changes the picture from 8 scattered marks to a 35-mark corridor down the channel. ALSO: the TAP READOUT now reads the band and the current off the same 0.56 km cell the ring is drawn on (fineLookup). Without that, 19.2% of drawn rings answered "nothing reported here" when tapped and 12.2% reported a LOWER severity than their own ring (measured, 1,191 ring cell-hours); it is now 100% agreement. Wind / wind-sea / fetch stay on the 2.2 km tile (a full 0.56 km readout was priced at ~9 MB and rejected), and the Quartering and Wave Height panels are deliberately UNTOUCHED - both are computed at 2.2 km FROM the 2.2 km current. physics.js UNCHANGED at sha1 db0083056cb1; the bake stamps that same hash into its manifest.
