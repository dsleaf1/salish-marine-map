 ...
 */
const CACHE_VERSION = "salishmap-shell-v25";   // 2026-08-14a: BAND 4 REPAIR + RING STABILITY + BOUNDS. (1) The top steepness ring (band 4, "whole spectrum breaking") was UNREACHABLE — specCore caps each component's S at 1/7 so the weighted mean could never reach the old 0.143 threshold; changed to fBrk>=1||S>=1/7 (physics.js sha1 CHANGED). (2) Ring placement now uses map.project() instead of latLngToContainerPoint for the decimation bucket key, so rings no longer shift when panning at fixed zoom. (3) maxBounds tightened from FULL_EXTENT.pad(0.12) to a custom rectangle [[46.85,-129.1],[51.75,-122.35]] so the map can't pan into inland areas east of the model domain. (4) About panel: resolution text corrected to "150–350 m". (5) Wet-gate fix: WIND_BORROW_DEG + nearestWetCell so steepness reads don't silently fail at coast cells.
